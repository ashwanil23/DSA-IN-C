...
struct node *IterativeSearch(struct node *temp, int key)
{
    while (temp != NULL)
    {
        if (temp->data == key)
...
{
    struct node *isPre = IterativeSearch(temp, key);
    if (isPre != NULL)
    {
        printf("Found :%d \n", isPre->data);
    }
...
// Iterative Search in BST Ends Here
...
int main()
{
    ...
    isPresent(rootNode1, 1);
    isPresent(rootNode, 1);
